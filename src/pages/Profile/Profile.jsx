...
function Profile() {
  ...
  const navigate = useNavigate();  // To redirect on error

  // Axios Interceptor for setting token in all requests
  useEffect(() => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (error.response && error.response.status === 404) {
        console.log('User not found, redirecting to error page.');
